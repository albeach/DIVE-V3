...
${DOCKER_CMD:-docker} exec "$CONTAINER_NAME" mongosh admin -u "$ADMIN_USER" -p "$ADMIN_PASSWORD" --tls --tlsAllowInvalidCertificates --quiet --eval "
try {
  ...
  try {
    ...
    if (e.message.includes('no replset config')) {
      ...
      // CRITICAL: Use container hostname for replica set member
      // This allows other containers to connect via Docker network DNS
      const hostname = '$CONTAINER_NAME';
      ...
      if (result.ok === 1) {
        print('✅ Replica set initialized successfully with host:', hostname + ':27017');
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    echo "  3. Check replica set status: ${DOCKER_CMD:-docker} exec dive-hub-mongodb mongosh admin -u admin -p PASSWORD --tls --tlsAllowInvalidCertificates --eval 'rs.status()'"
    echo ""
    exit 1
fi
