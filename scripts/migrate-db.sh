...
print_header() {
    echo -e "${BLUE}"
    echo "╔════════════════════════════════════════════════════════════╗"
    echo "║          DIVE V3 Database Migration                        ║"
    echo "╚════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
...
migrate_local() {
    echo -e "${BLUE}🔄 Running migrations (local)...${NC}"
...
EOF
    ...
    if [ -f "frontend/package.json" ] && grep -q "db:migrate" frontend/package.json; then
        ...
        echo -e "${YELLOW}⚠️  Drizzle migrations not configured${NC}"
...
migrate_k8s() {
    echo -e "${BLUE}🔄 Running migrations (Kubernetes)...${NC}"
