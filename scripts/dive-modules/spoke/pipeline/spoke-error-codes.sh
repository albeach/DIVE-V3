...
# sc2034-anchor
: "${SPOKE_ERROR_API_HEALTH:-}" "${SPOKE_ERROR_AUTH_FLOW:-}" "${SPOKE_ERROR_CANCELLED:-}" "${SPOKE_ERROR_CERT_COPY:-}" "${SPOKE_ERROR_CERT_GENERATION:-}" "${SPOKE_ERROR_CLIENT_SECRET:-}"
: "${SPOKE_ERROR_COMPOSE_GENERATE:-}" "${SPOKE_ERROR_COMPOSE_UP:-}" "${SPOKE_ERROR_CONFIG_GENERATE:-}" "${SPOKE_ERROR_CONNECTIVITY:-}" "${SPOKE_ERROR_CONTAINER_START:-}" "${SPOKE_ERROR_CONTAINER_UNHEALTHY:-}"
: "${SPOKE_ERROR_DATABASE_CHECK:-}" "${SPOKE_ERROR_DIRECTORY_SETUP:-}" "${SPOKE_ERROR_FEDERATION_REGISTER:-}" "${SPOKE_ERROR_FEDERATION_SETUP:-}" "${SPOKE_ERROR_FEDERATION_TEST:-}" "${SPOKE_ERROR_FEDERATION_VERIFY:-}"
: "${SPOKE_ERROR_GCP_AUTH:-}" "${SPOKE_ERROR_HEALTH_CHECK:-}" "${SPOKE_ERROR_HUB_NOT_FOUND:-}" "${SPOKE_ERROR_HUB_UNHEALTHY:-}" "${SPOKE_ERROR_INSTANCE_CONFLICT:-}" "${SPOKE_ERROR_KEYCLOAK_CONFIG:-}"
: "${SPOKE_ERROR_NETWORK_SETUP:-}" "${SPOKE_ERROR_OPAL_TOKEN:-}" "${SPOKE_ERROR_PROTOCOL_MAPPERS:-}" "${SPOKE_ERROR_ROLLBACK_FAILED:-}" "${SPOKE_ERROR_SECRET_LOAD:-}" "${SPOKE_ERROR_SECRET_SYNC:-}"
: "${SPOKE_ERROR_SECRET_VALIDATION:-}" "${SPOKE_ERROR_SERVICE_DEPENDENCY:-}" "${SPOKE_ERROR_SERVICE_TIMEOUT:-}" "${SPOKE_ERROR_STALE_CONTAINER:-}" "${SPOKE_ERROR_STATE_INVALID:-}" "${SPOKE_ERROR_TERRAFORM_APPLY:-}"
: "${SPOKE_ERROR_TERRAFORM_INIT:-}" "${SPOKE_ERROR_TIMEOUT:-}" "${SPOKE_ERROR_UNKNOWN:-}" "${SPOKE_ERROR_VOLUME_CREATE:-}"
