...
cmd_help() {
    ...
    echo ""
    echo -e "${BOLD}Core Commands (Instance-Aware):${NC}"
    ...
    echo ""
    echo -e "  ${YELLOW}⚠️  DEPRECATED (will be removed in v5.0):${NC}"
    echo "    ./dive --instance fra up       # ❌ Ambiguous - use explicit 'spoke up'"
    echo "    ./dive --instance fra down     # ❌ Ambiguous - use explicit 'spoke down'"
    echo "    ./dive --instance fra logs     # ❌ Ambiguous - use explicit 'spoke logs'"
    echo ""
    ...
    echo "  ./dive --instance fra spoke register --poll"
    echo ""
    echo "  # Check spoke health"
    echo "  ./dive --instance fra spoke health"
    echo "  ./dive --instance fra spoke verify    # 13-point check"
    echo ""
    ...
    echo ""
    echo -e "${BOLD}Command Routing Rules:${NC}"
    echo -e "  ${GREEN}✅ CORRECT Usage:${NC}"
    echo "    ./dive hub up                      # Start hub"
    echo "    ./dive --instance fra spoke up     # Start spoke"
    echo "    ./dive --instance fra spoke logs   # Spoke logs"
    echo ""
    echo -e "  ${RED}❌ DEPRECATED (Ambiguous):${NC}"
    echo "    ./dive --instance fra up           # Will route to hub (WRONG!)"
    echo "    ./dive --instance fra logs         # Will show hub logs (WRONG!)"
    echo ""
    echo -e "  ${YELLOW}⚠️  These deprecated patterns will be removed in v5.0${NC}"
    echo "     Always use explicit 'hub' or 'spoke' keywords."
    echo ""
