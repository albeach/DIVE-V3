...
log ""
log_warning "IMPORTANT: How to find your JWT token"
log ""
log "Your frontend uses DATABASE SESSION STRATEGY (NextAuth + PostgreSQL)"
log "This means the JWT token is NOT visible in browser cookies/storage."
log ""
log "The JWT is sent server-side from NextAuth → Backend API."
log ""
log "To capture the token, use ONE of these methods:"
log ""
log "METHOD 1 - From Browser Network Tab (EASIEST):"
...
log "  5. Navigate to any page (e.g., Documents or Resources)"
log "  6. Look for API requests to '/api/resources' or similar"
...
log ""
log "METHOD 2 - From Backend Logs:"
log "  1. Open a new terminal and run:"
log "     docker compose logs backend -f | grep 'Authorization\\|Bearer'"
log "  2. In your browser, navigate to a page (Documents, etc.)"
log "  3. Look for lines showing Bearer tokens in the logs"
log "  4. Copy the full token (starts with eyJ...)"
log ""
log "METHOD 3 - Skip token capture:"
log "  Press Enter to skip and use test token instead"
log ""
log "NOTE: The token should start with 'eyJ' and have 3 parts separated by dots (xxx.yyy.zzz)"
log "      Token length is usually 1000-2000+ characters"
log ""
