...
if [ "$CUSTOM_HOSTNAME" != "localhost" ]; then
    echo -e "${CYAN}🔐 Remote Access Setup (Required for Federation Partners):${NC}"
    echo ""
    echo -e "${YELLOW}⚠️  IMPORTANT: Federation partners MUST install your mkcert Root CA${NC}"
    echo "   to trust certificates from this DIVE V3 instance."
    echo ""
    echo "For remote clients to access DIVE V3:"
    echo ""
    echo "  1. ${YELLOW}Configure DNS or /etc/hosts on client machines:${NC}"
    SERVER_IP=$(ip route get 1 2>/dev/null | awk '{print $7}' | head -1 || echo '<your-server-ip>')
    echo "     ${SERVER_IP} ${CUSTOM_HOSTNAME}"
    echo ""
    echo "  2. ${YELLOW}Distribute YOUR mkcert Root CA:${NC}"
    echo "     ${BLUE}Location on this server:${NC} certs/mkcert/rootCA.pem"
    echo ""
    echo "  3. ${YELLOW}Clients install the Root CA:${NC}"
    echo ""
    echo "     ${BLUE}macOS:${NC}"
