...
resource "keycloak_oidc_identity_provider" "deu_realm_broker" {
  ...
  authorization_url = "${local.realm_urls.deu}${local.oidc_auth_path}"
  token_url         = "${local.realm_urls.deu}${local.oidc_token_path}"
  jwks_url          = "${local.realm_urls.deu}${local.oidc_certs_path}"
  user_info_url     = "${local.realm_urls.deu}${local.oidc_userinfo_path}"
  ...
  logout_url            = "${local.realm_urls.deu}${local.oidc_logout_path}"


  gui_order = "4"
}

resource "keycloak_custom_identity_provider_mapper" "deu_broker_username" {
  realm                    = keycloak_realm.dive_v3_broker.id
  identity_provider_alias  = keycloak_oidc_identity_provider.deu_realm_broker.alias
  name                     = "deu-username-from-uniqueID"
