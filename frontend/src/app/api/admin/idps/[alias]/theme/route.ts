/**
 * Admin IdP Theme API Route
 * 
 * Proxy for backend IdP theme endpoint
 * GET - Get theme
 * PUT - Update theme
 * DELETE - Delete theme
 */
...
export async function GET(
    ...
    try {
        ...
        const response = await fetch(`${BACKEND_URL}/api/admin/idps/${alias}/theme`, {
            method: 'GET',
            headers: {
        ...
        console.error('[AdminIdPThemeAPI] GET Error:', error);
        return NextResponse.json(
...
export async function PUT(
    ...
    try {
        ...
        console.error('[AdminIdPThemeAPI] PUT Error:', error);
        return NextResponse.json(
            { error: 'InternalError', message: error instanceof Error ? error.message : 'Internal server error' },
            { status: 500 }
        );
...
export async function DELETE(
    ...
    try {
        ...
        console.error('[AdminIdPThemeAPI] DELETE Error:', error);
        return NextResponse.json(
            { error: 'InternalError', message: error instanceof Error ? error.message : 'Internal server error' },
            { status: 500 }
        );
