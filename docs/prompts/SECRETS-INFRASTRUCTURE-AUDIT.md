...
│  dive-v3-keycloak-usa, dive-v3-mongodb-usa, etc.                │
│  dive-v3-client-secret-usa (NEW - synced from Terraform)        │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
...
┌─────────────────────────────────────────────────────────────────┐
│                    docker-compose.yml                            │
│  ALL secrets referenced as: ${VAR_NAME:?error message}          │
...
│  Same secrets used by Terraform (passwords match)                │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│                    TERRAFORM (Keycloak IaC)                      │
│  1. READS admin passwords FROM GCP (not from .tfvars)            │
│  2. Creates realms, clients, users                               │
│  3. WRITES generated client secrets TO GCP                       │
│  4. State file tracks what was created                           │
└─────────────────────────────────────────────────────────────────┘
```
...
|-------------|-------------------|---------|----------------|---------------------|----------|
| Keycloak Admin PW | ✅ dive-v3-keycloak-usa | ❌ Hardcoded different value | ❌ Reads from env (if set) | N/A | ❌ NO |
| Client Secret | ✅ dive-v3-keycloak-client-secret | N/A | ❌ HARDCODED | ✅ Generated fresh | ❌ NO |
| MongoDB PW | ✅ dive-v3-mongodb-usa | N/A | ❌ Partially env | N/A | ⚠️ MAYBE |
| Postgres PW | ✅ dive-v3-postgres-usa | ❌ Hardcoded | ❌ Reads from env | N/A | ❌ NO |
...
3. **HIGH**: Add Terraform → GCP sync for client_secret
