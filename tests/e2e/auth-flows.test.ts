...
const CONFIG = {
  ...
  testUsers: {
    ...
    fra: {
      username: 'bob.fr@defense.gouv.fr',
      password: 'TestPassword123!',
    ...
    gbr: {
      username: 'charlie.uk@mod.gov.uk',
      password: 'TestPassword123!',
      clearance: 'SECRET',
      country: 'GBR',
      realm: 'dive-v3-broker',
...
async function login(page: Page, user: typeof CONFIG.testUsers.usa): Promise<void> {
  ...
  await page.goto(CONFIG.baseUrls.frontend, { timeout: CONFIG.timeouts.navigation });
...
test.describe('Service Health Checks', () => {
  ...
  test('keycloak health check', async ({ request }) => {
    const response = await request.get(`${CONFIG.baseUrls.keycloak}/health/ready`, {
      failOnStatusCode: false,
    });
