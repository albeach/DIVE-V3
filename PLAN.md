...
| ACP-240 compliance evidence integrity | `backend/src/controllers/compliance.controller.ts:35` | Partial (2026-02-20) | Hardcoded replaced with runtime metrics (PR #700); OPAL integrity now dynamic + `standardsSummary` added (PR #715). Remaining: signed report payloads. |
...
| OPAL policy lifecycle governance security | `docker-compose.hub.yml:1164`, `docker-compose.hub.yml:1167`, `backend/src/routes/opal.routes.ts:331` | Partial (2026-02-20) | Auth enforced on publish/build (PR #692); SHA-256 hash + RS256 signing verified in CI (PR #715). Remaining: signed bundle policy gate in deployment pipeline. |
