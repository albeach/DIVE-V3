...
check_line_endings() {
    ...
    local crlf_files
    crlf_files=$(find "$PROJECT_ROOT" -type f \( -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" -o -name "*.json" -o -name "*.yml" -o -name "*.yaml" -o -name "*.md" -o -name "*.sh" -o -name "*.py" -o -name "*.java" -o -name "*.xml" -o -name "*.properties" -o -name "*.rego" -o -name "*.ftl" -o -name "*.css" -o -name "*.scss" -o -name "*.html" \) -exec grep -l $'\r' {} \; 2>/dev/null || true)
