...
async function main() {
    ...
    if (DRY_RUN) {
        ...
        for (let i = 1; i <= 10; i++) {
            ...
            console.log(`      Classification: ${doc.legacy.classification}`);
            console.log(`      COI: [${doc.legacy.COI.join(', ')}] (${doc.legacy.coiOperator})`);
            console.log(`      REL: [${doc.legacy.releasabilityTo.join(', ')}]`);
