...
const registrationSchema = z.object({
    ...
    // Skip IdP TLS validation during registration (e.g., Caddy ACME cert still provisioning)
...
router.post('/register', async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        // Skip IdP validation in test environments OR when the registering spoke explicitly requests it
        // (e.g., remote spokes behind Caddy where ACME cert provisioning may still be in progress)
        const skipValidation = process.env.NODE_ENV === 'test' || request.skipValidation === true;
 ...
 * Allows spoke deploy to fail fast with a clear error message.
 * No auth required — the auth code itself is the credential.
 */
router.get('/auth-code/:instanceCode/validate', async (req: Request, res: Response): Promise<void> => {
    try {
        const instanceCode = req.params.instanceCode.toUpperCase();
        const authCode = req.query.code as string;

        if (!authCode) {
            res.status(400).json({ valid: false, reason: 'missing_code', message: 'Query parameter "code" is required' });
            return;
        }
        ...
        if (!vaultToken) {
            res.status(503).json({ valid: false, reason: 'vault_unavailable', message: 'Cannot validate auth code — Vault not accessible' });
            return;
        }
        ...
        if (!vaultRes.ok) {
            res.json({ valid: false, reason: 'not_found', instanceCode, message: `No authorization record found for ${instanceCode}` });
            return;
        }

        const vaultData = vaultRes.data as { data: { data: Record<string, string> } };
        const authRecord = vaultData.data.data;

        if (authRecord.auth_code !== authCode) {
            res.json({ valid: false, reason: 'invalid_code', instanceCode, message: 'Authorization code does not match' });
            return;
        }

        if (authRecord.status === 'consumed') {
            res.json({ valid: false, reason: 'consumed', instanceCode, message: 'This authorization code has already been used' });
            return;
        }

        if (authRecord.status !== 'authorized') {
            res.json({ valid: false, reason: 'invalid_status', instanceCode, message: `Authorization record status: ${authRecord.status}` });
            return;
        }

        const expiresAt = authRecord.expires_at;
        if (expiresAt && new Date(expiresAt).getTime() <= Date.now()) {
            res.json({ valid: false, reason: 'expired', instanceCode, expiresAt, message: 'Authorization code has expired' });
            return;
        }
        ...
        res.json({
            ...
            authorizedBy: authRecord.authorized_by || 'unknown',
        });
    } catch (error) {
        logger.error('Auth code validation error', { error: error instanceof Error ? error.message : 'Unknown' });
        res.status(500).json({ valid: false, reason: 'internal_error', message: 'Auth code validation failed' });
    }
});
